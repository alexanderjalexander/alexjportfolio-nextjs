...
function obtainCols(cols:{}) {
    return Object.keys(cols)
        // @ts-ignore
        .map((col) => ({column: col, label: col.toUpperCase(), type: getColType(cols[col].dataType)}))
}
...
export const getAllSkillCategoryMappings = unstable_cache(
    async () => {
        ...
        return {
            columns: (Object.keys(data[0]) ?? ['skillId', 'skill', 'categoryId', 'category'])
                // @ts-ignore
                .map((col) => ({column: col, label: col.toUpperCase(), type: getColType(typeof data[0][col])})),
            data: data,
        }
    },
    ['skillCategory'],
